...
| **Broken lead notification** | Root-cause diagnosis (Process Builder), rebuild as Flow automation. Partner with Angel/James (Agentforce). | Week 1-4 | Not an ATLAS fix — infrastructure fix. But ATLAS alerts bypass the broken notification entirely (agent notifies directly via Teams). |
